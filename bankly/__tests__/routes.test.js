...
describe("POST /auth/login", function() {
  ...
  // TESTS BUG #1
  test("should not allow an icorrect username/password to log in", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u1",
...
describe("GET /users", function() {
  ...
  // TESTS BUG #2
  test("should return basic user info", async function() {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    expect(response.statusCode).toBe(200);
...
describe("PATCH /users/[username]", function() {
  ...
  // TESTS BUG #3
  test("should patch data if user is self", async function() {
    const response = await request(app)
      .patch("/users/u1")
  ...
  test("should disallowing patching non-existent fields", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, height: 62 });
    expect(response.statusCode).toBe(401);
  });
...
describe("authUser", function() {
  test("works: no token", async function() {
    const req = {body: {}};
    const res = { locals: {} };
